...
def model_fn(mode, features, labels, params):
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        ...
        input_images = tf.pad(tensor=input_images, paddings=[[0, 0], [margin, margin], [margin, margin], [0, 0]],
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        if key_restore_model is not None:
            ...
            pretrained_restorer = tf.compat.v1.train.Saver(var_list=[v for v in tf.compat.v1.global_variables()
    ...
    if prediction_type == PredictionType.CLASSIFICATION:
        ...
        prediction_labels = tf.argmax(input=network_output, axis=-1, name='label_preds')
    ...
    if mode in [tf.estimator.ModeKeys.TRAIN, tf.estimator.ModeKeys.EVAL]:
        ...
        if prediction_type == PredictionType.CLASSIFICATION:
            ...
            with tf.name_scope("loss"):
                per_pixel_loss = tf.nn.softmax_cross_entropy_with_logits(logits=network_output,
                                                                         labels=tf.stop_gradient(onehot_labels), name='per_pixel_loss')
                if training_params.focal_loss_gamma > 0.0:
                    # Probability per pixel of getting the correct label
                    probs_correct_label = tf.reduce_max(input_tensor=tf.multiply(prediction_probs, onehot_labels))
                ...
                if training_params.weights_labels is not None:
                    weight_mask = tf.reduce_sum(
                        input_tensor=tf.constant(np.array(training_params.weights_labels, dtype=np.float32)[None, None, None]) *
            ...
            per_pixel_loss = tf.math.squared_difference(labels, network_output, name='per_pixel_loss')
        elif prediction_type == PredictionType.MULTILABEL:
            with tf.name_scope('sigmoid_xentropy_loss'):
                ...
                if training_params.weights_labels is not None:
                    weight_mask = tf.maximum(
                        tf.reduce_max(input_tensor=tf.constant(
        ...
        with tf.name_scope('Loss'):
            def _fn(_in):
                output, shape = _in
                return tf.reduce_mean(input_tensor=output[margin:shape[0] - margin, margin:shape[1] - margin])

            per_img_loss = tf.map_fn(_fn, (per_pixel_loss, input_shapes), dtype=tf.float32)
            loss = tf.reduce_mean(input_tensor=per_img_loss, name='loss')
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        if training_params.exponential_learning:
            ...
            learning_rate = tf.compat.v1.train.exponential_decay(training_params.learning_rate, global_step, decay_steps=200,
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        with tf.name_scope('summaries'):
            ...
            tf.compat.v1.summary.scalar('losses/loss_per_batch', loss)
            tf.compat.v1.summary.scalar('losses/regularized_loss', regularized_loss)
            if prediction_type == PredictionType.CLASSIFICATION:
                tf.compat.v1.summary.image('output/prediction',
                                 tf.compat.v1.image.resize_images(class_to_label_image(prediction_labels, classes_file),
                                                        tf.cast(tf.shape(input=network_output)[1:3] / 3, tf.int32)),
                                 max_outputs=1)
                if model_params.n_classes == 3:
                    tf.compat.v1.summary.image('output/probs',
                                     tf.compat.v1.image.resize_images(prediction_probs[:, :, :, :],
                                                            tf.cast(tf.shape(input=network_output)[1:3] / 3, tf.int32)),
                                     max_outputs=1)
                if model_params.n_classes == 2:
                    tf.compat.v1.summary.image('output/probs',
                                     tf.compat.v1.image.resize_images(prediction_probs[:, :, :, 1:2],
                                                            tf.cast(tf.shape(input=network_output)[1:3] / 3, tf.int32)),
                                     max_outputs=1)
            elif prediction_type == PredictionType.REGRESSION:
                summary_img = tf.nn.relu(network_output)[:, :, :, 0:1]  # Put negative values to zero
                tf.compat.v1.summary.image('output/prediction', summary_img, max_outputs=1)
            elif prediction_type == PredictionType.MULTILABEL:
                ...
                tf.compat.v1.summary.image('output/prediction_image',
                                 tf.compat.v1.image.resize_images(labels_visualization,
                                                        tf.cast(tf.shape(input=labels_visualization)[1:3] / 3, tf.int32)),
                                 max_outputs=1)
                class_dim = prediction_probs.get_shape().as_list()[-1]
                for c in range(0, class_dim):
                    tf.compat.v1.summary.image('output/prediction_probs_{}'.format(c),
                                     tf.compat.v1.image.resize_images(prediction_probs[:, :, :, c:c + 1],
                                                            tf.cast(tf.shape(input=network_output)[1:3] / 3, tf.int32)),
                                     max_outputs=1)
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
        if prediction_type == PredictionType.CLASSIFICATION:
            metrics = {
                'eval/accuracy': tf.compat.v1.metrics.accuracy(labels, predictions=prediction_labels),
                'eval/mIOU': tf.compat.v1.metrics.mean_iou(labels, prediction_labels, num_classes=model_params.n_classes,)
                                                 # weights=tf.cast(training_params.weights_evaluation_miou, tf.float32))
            }
        elif prediction_type == PredictionType.REGRESSION:
            metrics = {'eval/accuracy': tf.compat.v1.metrics.mean_squared_error(labels, predictions=prediction_labels)}
        elif prediction_type == PredictionType.MULTILABEL:
            metrics = {'eval/MSE': tf.compat.v1.metrics.mean_squared_error(tf.cast(labels, tf.float32),
                                                                 predictions=prediction_probs),
                       'eval/accuracy': tf.compat.v1.metrics.accuracy(tf.cast(labels, tf.bool),
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        ...
        if 'original_shape' in features.keys():
            with tf.name_scope('ResizeOutput'):
                ...
                for k, v in predictions.items():
                    ...
                    v2 = tf.compat.v1.image.resize_images(v2, features['original_shape'],
        ...
        export_outputs[tf.saved_model.DEFAULT_SERVING_SIGNATURE_DEF_KEY] = export_outputs['output']
    else:
        export_outputs = None
